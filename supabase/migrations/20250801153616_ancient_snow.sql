/*
  ...
  1. Problem
    - Users can sign up via Supabase Auth but no profile is created in user_profiles table
    - This causes authentication to fail because useAuth expects a profile

  2. Solution
    - Create a database trigger that automatically creates user profile and wallet
    - Trigger fires when a new user is inserted into auth.users table
    - Creates profile, wallet, and initial token transaction automatically
