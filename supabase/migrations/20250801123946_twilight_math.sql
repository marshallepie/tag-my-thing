/*
  ...
  1. Core Tables
    - user_profiles: Extended user information with NOK and business features
    - user_wallets: TMT token management
    - assets: Tagged assets with archiving capabilities
    - next_of_kin: NOK relationships for legacy planning
  ...
  3. Referral System
    ...
    - referral_settings: Configurable reward structure
    
  4. Business Features
    - products: Product registration for verification
    - scan_events: Product scan tracking
  ...
  6. Security
    ...
    - SECURITY DEFINER functions to avoid RLS conflicts
...
-- Create safe role checking function to avoid RLS recursion
...
-- User wallets table (TMT token management)
...
-- Assets table (tagged assets with comprehensive metadata)
...
-- =============================================
-- NEXT-OF-KIN SYSTEM TABLES
-- =============================================

-- Next of kin table (NOK relationships)
...
-- Asset NOK assignments table (with Dead Man's Switch)
...
-- Token transactions table (complete transaction history)
...
-- Subscription plans table (business subscription tiers)
...
-- Token packages table (token purchase options)
...
-- Referral rewards table (token rewards for referrals)
...
-- Referral settings table (configurable reward structure)
...
-- Products table (business product registration)
...
-- Scan events table (product verification tracking)
...
-- Bug reports table (in-app bug reporting)
...
CREATE INDEX IF NOT EXISTS idx_user_profiles_referral_code ON user_profiles(referral_code) WHERE referral_code IS NOT NULL;
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM user_profiles 
    WHERE id = current_user_id AND is_business_user = true
  ) THEN
    RETURN jsonb_build_object(
      'success', false,
      'error', 'Only business users can register products'
    );
  END IF;
  ...
  VALUES (p_product_name, p_serial_number, p_description, current_user_id)
...
EXCEPTION WHEN OTHERS THEN
  RETURN jsonb_build_object(
    'success', false,
    'error', 'Failed to register product: ' || SQLERRM
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Function to verify product scan
...
BEGIN
  ...
  -- Log the scan event
  INSERT INTO scan_events (serial_number, ip_address, location, device_info, user_id)
  VALUES (p_serial_number, p_ip_address, p_location, p_device_info, auth.uid());
  ...
  -- Check for suspicious activity (more than 10 scans in last 24 hours)
  SELECT COUNT(*) INTO recent_scans
  FROM scan_events
  WHERE serial_number = p_serial_number
  AND scanned_at >= now() - interval '24 hours';

  IF recent_scans > 10 THEN
    flagged := true;
  END IF;
  ...
  FROM (
    SELECT scanned_at, location, ip_address, device_info
    FROM scan_events
    WHERE serial_number = p_serial_number
    ORDER BY scanned_at DESC
    LIMIT 10
...
DECLARE
  ...
  product_owner uuid;
BEGIN
  -- Verify the current user owns this product
  SELECT business_user_id INTO product_owner
  FROM products
  WHERE serial_number = p_serial_number;
...
EXCEPTION WHEN OTHERS THEN
  RETURN jsonb_build_object(
    'success', false,
    'error', 'Failed to get scan history: ' || SQLERRM
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- Function to check business subscription limits
...
-- Business documents storage bucket
INSERT INTO storage.buckets (id, name, public) 
...
INSERT INTO subscription_plans (name, token_limit, price_gbp, price_xaf, billing_interval, features, active) VALUES
  ...
  ('professional', 1000, 8.99, 6742, 'monthly', ARRAY[
  ...
  ('enterprise', 10000, 49.99, 37492, 'monthly', ARRAY[
...
INSERT INTO token_packages (name, token_amount, bonus_tokens, price_gbp, price_xaf, active) VALUES
  ('Starter Pack', 100, 0, 1.00, 750, true),
  ('Power Pack', 500, 0, 4.50, 3375, true),
  ('Mega Pack', 5000, 0, 39.99, 29992, true)
ON CONFLICT (name) DO UPDATE SET
...
-- Create test influencer user if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    INSERT INTO user_wallets (user_id, balance) 
    VALUES ('8056c771-42be-40e2-942a-28950b3b1ae6', 100);
...
COMMENT ON TABLE user_profiles IS 'Extended user information with NOK and business features';
COMMENT ON TABLE user_wallets IS 'TMT token wallet management';
COMMENT ON TABLE assets IS 'Tagged assets with archiving capabilities';
...
COMMENT ON TABLE subscription_plans IS 'Business subscription tiers with features';
COMMENT ON TABLE token_packages IS 'Token purchase options';
...
COMMENT ON TABLE referral_settings IS 'Configurable referral reward structure';
...
COMMENT ON FUNCTION is_user_role(text) IS 'SECURITY DEFINER function to safely check user roles without triggering RLS recursion';
