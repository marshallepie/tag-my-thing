  ...
  useEffect(() => {
    const handleAuthCallback = async () => {
      ...
      try {
        ...
        // Apply referral regardless of whether refCode exists
        console.log('About to apply referral:', refCode || 'default (epie@marshallepie.com)');
        setMessage(refCode ? 'Email verified! Processing your referral...' : 'Email verified! Assigning to default referrer...');
        
        try {
          const { data: rpcData, error: rpcErr } = await supabase.rpc('apply_referral_on_signup', {
            p_new_user_id: user.id,
            p_referral_code: refCode || '', // Pass empty string if no code - function will handle default
          ...
          } else if (rpcData?.default_referrer_used) {
            console.log('Default referrer (epie@marshallepie.com) assigned successfully');
            setMessage('Email verified and assigned to referral system! Redirecting...');
          } else if (rpcData?.success) {
            console.log('Specific referral applied successfully from email verification');
            setMessage('Email verified and referral applied! Redirecting...');
          } else {
            console.log('Referral processing completed with unknown status');
            setMessage('Email verified! Redirecting to your dashboard...');
          }
        } catch (ex) {
